...
GKD_Vision 是一个高性能的 RoboMaster 比赛机器人视觉系统，基于 OpenRM 框架开发，整合了 TJURM 项目的功能特性并进行了优化改进。旨在提供精确的目标检测、识别和跟踪功能，支持多种机器人兵种。
...
### YOLO 分类器优化 🧠
- 解决 ID 识别不准问题（哨兵常被误识别为X2/X3/X4/X5）
- 实现基于装甲板大小的智能修正
- 基于优先级的攻击策略
...
1. **模型文件未找到** - 确认模型文件位于 `include/model/yolov7/`
2. **相机连接失败** - 检查相机驱动和权限设置
3. **配置文件错误** - 确认对应兵种配置已正确复制

### 调试模式

- 使用 `-s` 参数启用图像显示进行视觉调试
- 使用 `-v` 参数获取详细输出信息
- 检查 `/etc/openrm/forward_config/` 配置文件

## 🤝 贡献

欢迎提交 Issue 和 Pull Request 来帮助改进 GKD_Vision！
...
**GKD_Vision** - 为 RoboMaster 比赛而生的高性能视觉系统！🎯🤖