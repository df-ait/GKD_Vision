#ifndef __VISIONLIB_KALMAN_INTERFACE_TRACK_QUEUE_V4_H__
#define __VISIONLIB_KALMAN_INTERFACE_TRACK_QUEUE_V4_H__
#include <memory>
#include <vector>
...
struct TrackQueueV4_FuncA {
    ...
    void operator()(const T x0[8], T x1[8]) {
        ...
        x1[1] = x0[1] + dt * x0[3] * ceres::sin(x0[5]) + 0.5 * dt * dt * x0[7] * ceres::sin(x0[5]); //这tm绝对不对傻逼TJU
        x1[2] = x0[2] + dt * x0[4];
        x1[3] = x0[3] + dt * x0[7];
        x1[4] = x0[4];
        x1[5] = x0[5] + dt * x0[6];
